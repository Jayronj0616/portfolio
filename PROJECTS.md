...
| BUGS Auto Quality Cars | 🟢 Live | bugs-auto-quality-cars.vercel.app | github.com/Jayronj0616/bugs-auto-quality-cars | Freelance work — dealership storefront + admin dashboard for a real client, running the client's actual inventory. Verified end to end against production per the project's own `docs/PROGRESS.md`. Cover image pending — Jayron is capturing his own screenshots (folder reserved at `public/images/bugs-auto/`). |
| AttendFlow AI | ⚫ Archived | attendflow-ai.vercel.app | github.com/Jayronj0616/AttendFlow_Ai | Personal project. AI-assisted attendance correction — deterministic business rules (not the AI) decide auto-apply vs. escalate to HR, full audit trail. Deployment is real and verified per the project's own `docs/progress.md`, but marked archived rather than live: the natural-language extraction step is still a documented placeholder pending an LLM key. `live_url` stays set, so the card still links out — only the badge changed. Cover image pending — Jayron is capturing his own screenshots (folder reserved at `public/images/attendflow/`). |
...
- [ ] Cover/gallery screenshots for BUGS Auto Quality Cars and AttendFlow AI — Jayron is capturing his own. Drop the main shot as `landing.jpg`/`.png` in `public/images/bugs-auto/` or `public/images/attendflow/` and set it as `cover_image` in both `fallbackProjects.js` and the live Supabase `projects` table.
